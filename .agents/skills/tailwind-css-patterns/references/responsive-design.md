...
Enable class-based dark mode in your CSS (v4 has no `tailwind.config.js`):

```css
/* src/styles.css */
@import "tailwindcss";

/* Toggle dark mode by adding the `dark` class to a parent (e.g. <html>) */
@custom-variant dark (&:where(.dark, .dark *));
```

By default v4 follows the OS via `prefers-color-scheme`; the variant above switches it to the `dark` class strategy.
...
3. **Respect system preference**: v4 follows `prefers-color-scheme` by default — only add the `@custom-variant dark` (class strategy) when you need a manual toggle
