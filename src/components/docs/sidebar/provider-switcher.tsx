...
export function ProviderSwitcher() {
  ...
  const selectProvider = (provider: Provider) => {
    ...
    // Try to hold your place across the switch: /docs/recharts/bar-chart/static →
    // /docs/echarts/bar-chart/static. The target rarely has a matching page yet, so
    // the provider index is the fallback — and the only destination we can promise
    // exists. Once ECharts has real pages, prefer swapping the segment and letting
    // a 404 fall back here.
      ...
      <SidebarMenuItem>
        <DropdownMenu>
          <DropdownMenuTrigger asChild>
            ...
            >
              <ProviderIcon provider={displayed.id} />
