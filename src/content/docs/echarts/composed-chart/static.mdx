  ...
  <TabsPanel value="manual">
    <Steps>
      ...
      <Step>
        ...
        <StepDescription>
          Create the folder `evilcharts` with a `charts` subfolder in your `components` directory, then copy the code below into a new `echarts-composed-chart.tsx` file there. Unlike the Recharts twin, this component is self-contained — it ships everything ECharts needs in one file, so there are no extra `ui` sub-components to add.
        </StepDescription>
        <StepContent>
          <ComponentSource
