  ...
  <TabsPanel value="manual">
    <Steps>
      ...
      <Step>
        ...
        <StepDescription>
          Create the folder `evilcharts` with a `charts` subfolder in your `components` directory, then paste the radar-chart code into a new `echarts-radar-chart.tsx` file there. Unlike the Recharts twin, this component is self-contained — everything ECharts needs ships in one file, with no extra `ui` sub-components to add.
        </StepDescription>
        <StepContent>
          <ComponentSource
