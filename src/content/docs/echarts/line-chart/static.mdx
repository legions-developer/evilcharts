...
The ECharts line chart is composable, sharing the Recharts twin's API shape. `<EChartsLineChart>` is the container; compose only the parts you need as its children — `<Grid>`, `<XAxis>`, `<YAxis>`, `<Legend>`, `<Tooltip>`, and one or more `<Line>`. Each `<Line>` carries its own `strokeVariant`, `curveType`, `glowing`, `enableBufferLine`, and `isClickable`, so one chart can mix stroke styles and make only some series interactive.
