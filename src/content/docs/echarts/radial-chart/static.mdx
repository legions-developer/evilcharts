  ...
  <TabsPanel value="manual">
    <Steps>
      ...
      <Step>
        ...
        <StepDescription>
          In your `components` directory, create `evilcharts` → `charts`, then paste the radial-chart code into a new `echarts-radial-chart.tsx` file there. Unlike the Recharts twin, this component is self-contained — it ships everything ECharts needs in one file, so there are no extra `ui` sub-components to add.
        </StepDescription>
        <StepContent>
          <ComponentSource
...
The ECharts radial chart is composible, sharing the Recharts twin's API shape. `<EChartsRadialChart>` is the container, and every part hangs off it as a compound member — `<EChartsRadialChart.Legend>`, `<EChartsRadialChart.Tooltip>`, and a `<EChartsRadialChart.RadialBar>` — so a single import gives you the whole chart. `<RadialBar>` carries its own `glowingBars` and `isClickable`, so styling and interactivity live with the series.
...
<EChartsRadialChart data={data} nameKey="browser" config={chartConfig} variant="full">
  ...
  <EChartsRadialChart.RadialBar dataKey="visitors" isClickable glowingBars={["chrome"]} />
</EChartsRadialChart>
```
