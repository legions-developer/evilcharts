  ...
  <TabsPanel value="manual">
    <Steps>
      ...
      <Step>
        ...
        <StepDescription>
          In your `components` directory, create an `evilcharts` folder with a `charts` subfolder and an `echarts` folder inside. Paste the code below into a new file there. Unlike the Recharts twin, this component is self-contained — everything ECharts needs ships in one file, so there are no extra `ui` sub-components to add.
        </StepDescription>
        <StepContent>
          <ComponentSource
...
The ECharts pie chart is composible, sharing the Recharts twin's API shape. `<EChartsPieChart>` is the container; compose the parts you need — `<Legend>`, `<Tooltip>`, `<Background>`, and one `<Pie>` — as its children. The `<Pie>` carries its own shape props (`innerRadius`, `paddingAngle`, `cornerRadius`, …) and an `isClickable` flag.
