  ...
  <TabsPanel value="manual">
    <Steps>
      ...
      <Step>
        ...
        <StepDescription>
          In your `components` directory, create an `evilcharts` folder with a `charts` subfolder inside. Paste the code below into a new `echarts-pie-chart.tsx` file there. Unlike the Recharts twin, this component is self-contained — everything ECharts needs ships in one file, so there are no extra `ui` sub-components to add.
        </StepDescription>
        <StepContent>
          <ComponentSource
