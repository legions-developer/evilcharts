  ...
  <TabsPanel value="manual">
    <Steps>
      ...
      <Step>
        ...
        <StepDescription>
          In your `components` directory, create `evilcharts`, then `charts`, then `echarts`, each nested inside the last, and paste the area-chart code into a new file there. Unlike the Recharts twin, this component is self-contained — everything ECharts needs ships in one file, so there are no extra `ui` sub-components to add.
        </StepDescription>
        <StepContent>
          <ComponentSource
...
The ECharts area chart is composable, matching the Recharts twin's API. `<EChartsAreaChart>` is the container; compose the parts you need — `<Grid>`, `<XAxis>`, `<YAxis>`, `<Legend>`, `<Tooltip>`, and one or more `<Area>` — as children. Each `<Area>` sets its own `variant`, `strokeVariant`, and `isClickable`, so one chart can mix fills, strokes, and selective interactivity.
