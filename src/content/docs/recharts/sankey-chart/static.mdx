    ...
    <Steps>
      ...
      <Step>
        <StepTitle>Copy the code below into your project.</StepTitle>
         <StepDescription>
          Create an `evilcharts` folder with a `charts` subfolder inside your `components` directory, then copy the base sankey-chart code into a new file there.
        </StepDescription>
        <StepContent>
          <ComponentSource
...
A compound component: `<EvilSankeyChart />` is the container; `<Node />`,
`<Link />`, and `<Tooltip />` compose as children. Render only the parts you need.
