...
<CodeTabs>
  ...
  <TabsPanel value="cli">
    <CliBlock commands={["@evilcharts/recharts-area-chart"]} />
  </TabsPanel>
...
The area chart is composable. `<EvilAreaChart>` is the container; drop in only the parts you need — `<Grid>`, `<XAxis>`, `<YAxis>`, `<Legend>`, `<Tooltip>`, and one or more `<Area>` — as children. Each `<Area>` owns its `variant`, `strokeVariant`, and `isClickable`, so one chart can mix fills, strokes, and per-series interactivity.
