...
links:
  github: https://github.com/legions-developer/evilcharts/blob/main/src/registry/charts/recharts-radial-chart.tsx
...
<CodeTabs>
  ...
  <TabsPanel value="cli">
    <CliBlock commands={["@evilcharts/recharts-radial-chart"]} />
  </TabsPanel>
      ...
      <Step>
        ...
        <StepContent>
          <ComponentSource
            ...
            title="components/evilcharts/charts/recharts-radial-chart.tsx"
          />
        </StepContent>
      </Step>
       <Step>
...
The radial chart is composable. `<EvilRadialChart>` is the container, and every part hangs off it as a compound member — `<EvilRadialChart.Legend>`, `<EvilRadialChart.Tooltip>`, and a `<EvilRadialChart.RadialBar>` — so a single import gives you the whole chart. `glowingBars` and `isClickable` live on `<EvilRadialChart.RadialBar>`, so styling and interactivity stay with the series.
...
<EvilRadialChart data={data} nameKey="browser" config={chartConfig} variant="full">
  ...
  <EvilRadialChart.RadialBar dataKey="visitors" isClickable glowingBars={["chrome"]} />
</EvilRadialChart>
```
...
<Alert> 
  <AlertContent>
    Use the `glowingBars` prop on `<RadialBar>` to add a subtle glow. Pass an array of bar names (values from your `nameKey` field) to choose which bars glow.
  </AlertContent>
</Alert>
...
<ApiTable>
  ...
  <ApiRow name="chartProps" type="ComponentProps<typeof RadialBarChart>">
    Props forwarded to the underlying Recharts RadialBarChart. See the <Link href="https://recharts.github.io/en-US/api/RadialBarChart/" _blank>Recharts RadialBarChart docs</Link> for options.
  </ApiRow>
</ApiTable>

<ApiHeading>RadialBar</ApiHeading>

The radial bar series — each data row becomes one bar. It scopes its own glow filters, so effects never collide with other charts on the page.
...
<ApiTable>
  ...
  <ApiRow name="glowingBars" type="string[]" default="[]">
    Bar names (values from your `nameKey` field) that get a smooth outer glow.
  </ApiRow>
  <ApiRow name="radialBarProps" type='Omit<ComponentProps<typeof RadialBar>, "dataKey">'>
    Props forwarded to the underlying Recharts RadialBar. See the <Link href="https://recharts.github.io/en-US/api/RadialBar/" _blank>Recharts RadialBar docs</Link> for options.
  </ApiRow>
</ApiTable>
