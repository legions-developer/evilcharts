...
The radial chart is composable. `<EvilRadialChart>` is the container; add only the parts you need — `<Legend>`, `<Tooltip>`, and a `<RadialBar>` — as children. `glowingBars` and `isClickable` live on `<RadialBar>`, so styling and interactivity stay with the series.
...
Add `isClickable` to `<RadialBar>` (and `<Legend>`) to make bars selectable, then handle selection with the `onSelectionChange` callback on `<EvilRadialChart>`:
