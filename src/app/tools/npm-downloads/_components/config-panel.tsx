...
import {
  ...
  MIN_RING_WIDTH,
  STROKE_VARIANTS,
} from "@/lib/npm-downloads/query-schema";
import type {
  AxisType,
  BackgroundPattern,
  ChartType,
...
import type { NpmDownloadsConfig } from "../_lib/state";
import { PackageInputList } from "./package-input-list";

interface ConfigPanelProps {
  config: NpmDownloadsConfig;
  onChange: (config: NpmDownloadsConfig) => void;
}
...
/** Download-metric options, in the order shown in the select. */
const METRIC_OPTIONS: { value: DownloadMetric; label: string }[] = [
  { value: "daily", label: "Daily" },
  { value: "rolling7", label: "7-day average" },
  { value: "weekly", label: "Weekly" },
  { value: "monthly", label: "Monthly" },
  { value: "cumulative", label: "Cumulative" },
];

/** Packages + appearance config. */
export function ConfigPanel({ config, onChange }: ConfigPanelProps) {
  const update = (patch: Partial<NpmDownloadsConfig>) => onChange({ ...config, ...patch });

  // Line plots the download trend; bar/radial/pie compare total downloads. The
  // axis and stroke controls only apply to a subset, so several rows are
  // conditional.
    ...
    <div className="flex flex-col gap-8">
      ...
      <Section title="Config">
        <Card>
          <Row
            label="Metric"
            description="Daily downloads, a 7-day average, weekly/monthly buckets, or a running total."
          >
            <Select
              value={config.metric}
              onValueChange={(metric) => update({ metric: metric as DownloadMetric })}
            >
          ...
          <Row label="Chart type" description="Download trend, or a bar/radial/pie total.">
            <Select
          ...
          {isLine && (
            <Row label="X axis" description="Plot downloads by calendar date or package age.">
              <Select
                value={config.axis}
          ...
          {isLine && (
            <Row
              label="Axis labels"
              description="Show axis titles beside the date and download ticks."
              htmlFor="npm-axis-labels"
            >
              <Switch
                id="npm-axis-labels"
