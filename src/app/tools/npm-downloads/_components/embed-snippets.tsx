...
interface EmbedSnippetsProps {
  /** Relative /api/npm-downloads URL. */
  embedUrl: string;
...
export function EmbedSnippets({ embedUrl, origin, fetchSvg, hasPackages }: EmbedSnippetsProps) {
  if (!hasPackages) {
    return (
      <div className="text-muted-foreground flex aspect-video items-center justify-center p-6 text-xs">
        Add a package below to generate an embeddable chart.
      </div>
    );
  }
